(ns dev
  (:require
   ...
   [nedap.components.assets.component :refer :all]))

(set-refresh-dirs "src" "test" "dev")

(defn suite []
  (refresh)
  (run-all-tests #".*\.nedap\.components\.assets.*"))

(defn unit []
  (refresh)
  (run-all-tests #"unit\.nedap\.components\.assets.*"))

(defn slow []
  (refresh)
  (run-all-tests #"integration\.nedap\.components\.assets.*"))
